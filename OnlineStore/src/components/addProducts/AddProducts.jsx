...
const AddProducts = ({ items, click, removeItem, setAddedItem }) => {
  ...
  return (
    <div ref={showDivRef} className="addproducts__container h-screen w-full fixed top-0 right-0 flex animat">
      <div className="h-screen md:w-[70%]">
      ...
      <div className="md:w-[30%] w-full md:bg-slate-200 bg-slate-200">
        <div className="right-side-header flex justify-between items-center">
          <h1>
            Shopping <span className="total-items">{items.length}</span>
            {items.length <= 1 ? " item" : " items"}
          </h1>
          <button
            className="remove-item-btn bg-red-500 px-2 text-xl rounded-lg shadow-sm"
            onClick={() => {
